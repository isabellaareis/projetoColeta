...
<body>
    ...
    <h1>Excluir Residuo</h1>
    <form method="post" class="excluir-form">
        <label for="tCodigo">Informe um Código</label>
        <input type="text" name="tCodigo" id="tCodigo" placeholder="Insira um código" required/>
        <button type="submit">Excluir
            <?php
                ...
                $consultar = new Excluir();
            ?>
        </button>
    </form>
    <?php
    if (isset($_POST['tCodigo'])) {
        $codigo = $_POST['tCodigo']; // Defina a variável corretamente
        echo $consultar->excluirResiduo($conexao, $codigo); // Passe a variável $codigo
    } else {
