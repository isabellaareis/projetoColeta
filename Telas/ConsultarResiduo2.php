...
<body>
    ...
    <h1>Consultar Residuos</h1>
    <form method="post" class="consult-form">
        <label for="tData">Informe uma Data ou um Código</label>
        <input type="text" name="tData" id="tData" placeholder="Digite a data ou código" required/>
    ...
    <?php
        if (isset($_POST['tData'])) {
            $conexao = new Conexao();
            $dataOuCodigo = $_POST['tData'];

            // Verificar se é uma data válida (formato dd/mm/yyyy)
            $dataValida = preg_match("/^\d{2}\/\d{2}\/\d{4}$/", $dataOuCodigo);
            $codigoValido = is_numeric($dataOuCodigo);  // Verifica se é um número (código)

            $consultar = new Consultar();

            if ($dataValida) {
                // Se for uma data válida no formato dd/mm/yyyy
                echo $consultar->consultarResiduo($conexao, $dataOuCodigo, "");
            } elseif ($codigoValido) {
                // Se for um código válido (número)
                echo $consultar->consultarResiduo($conexao, "", $dataOuCodigo);
            } else {
                echo "Por favor, informe uma data ou um código válido.";
            }
        } else {
